...
/**
 * Compares two versions generated by {@link parseVersion} as expected by
 * Array#sort, where a negative value indicates `a < b`, a positive value
 * indicates `a > b`, and `0` indicates `a = b`
...
export function compareVersion(a: number[], b: number[]): number {
  ...
  for (let i = 0; i < length; i++) {
    // Assume a number would be 0 if not present
